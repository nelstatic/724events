...
const Page = () => {
  const { data } = useData(); // Accès uniquement à `data`

  // Trouve le dernier événement basé sur les dates
  const lastEvent =
        ...
        <div className="col presta">
          ...
          {lastEvent && (
            <EventCard
              ...
              title={lastEvent.title || "Titre non disponible"}
              date={lastEvent.date ? new Date(lastEvent.date) : new Date()}
              small
              label={lastEvent.type || "Non catégorisé"}
            />
          )}
        </div>
